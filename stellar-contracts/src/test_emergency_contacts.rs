...
#[test]
fn test_emergency_contacts_authorized_responder_can_read() {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register_contract(None, PetChainContract);
    let client = PetChainContractClient::new(&env, &contract_id);
    ...
    // Owner adds responder to allowlist
    client.add_emergency_responder(&pet_id, &responder);

    // Authorized responder can read
...
fn test_emergency_contacts_unauthorized_read_fails() {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register_contract(None, PetChainContract);
    let client = PetChainContractClient::new(&env, &contract_id);
    ...
    // Stranger (not in allowlist) must be rejected
    client.get_emergency_contacts(&pet_id, &stranger);
}

#[test]
#[should_panic(expected = "NotAuthorizedResponder")]
fn test_emergency_contacts_removed_responder_cannot_read() {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register_contract(None, PetChainContract);
    let client = PetChainContractClient::new(&env, &contract_id);
